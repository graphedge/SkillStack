...
It does not replace Board personas: [board/workflow.md](../board/workflow.md) defines three lanes (Fast path, Change proposal, Review sequence) plus lane lock. This protocol does not add lanes. Linked `recursive-decomposition` and `manifest-generation` files in this snapshot are NON-CANON customer fill-ins, not required hops.
...
MUST NOT run the Board sequence or the proposal template below. MUST NOT escalate routine work into a Board play. Bounded suggestions use Change proposal in [board/workflow.md](../board/workflow.md): paste for an agent on the private operator source (SkillStack2); MUST NOT open a pull request or edit this snapshot directly. Unnamed lanes are locked.
