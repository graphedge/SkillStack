...
- **Change proposal (MUST):** Bounded suggestions use Change proposal in [board/workflow.md](board/workflow.md). Output a paste for an agent on the private operator source (SkillStack2). MUST NOT open a pull request or edit this snapshot directly. Unnamed lanes are locked; lane choice lives in workflow.md.
